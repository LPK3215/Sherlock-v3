...
def _apply_realtime_runtime_context(input_context: dict, meta: dict | None) -> None:
    meta = meta or {}
    if meta.get("source") != "realtime":
        return
    session_id = str(meta.get("realtime_session_id") or "").strip()
    if not session_id:
        raise ValueError("实时运行缺少 realtime_session_id")
    input_context["realtime_session_id"] = session_id
...
async def stream_agent_chat(
    *,
    ...
    _apply_realtime_runtime_context(input_context, meta)
