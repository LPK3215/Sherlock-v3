...
async def resolve_configured_runtime_tools(context) -> list[Any]:
    ...
    buildin_tools = {
        tool.name: tool for category in ("buildin", "learning") for tool in get_tool_instances_by_category(category)
    }
    ...
    if getattr(context, "realtime_session_id", None):
        for tool in get_tool_instances_by_category("realtime"):
            if tool.name in selected_tool_names:
                continue
            selected_tools.append(tool)
            selected_tool_names.add(tool.name)
