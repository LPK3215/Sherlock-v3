...
@pytest.mark.asyncio
async def test_ensure_business_schema_creates_domain_record_tables_and_indexes():
    manager = PostgresManager()
    original_initialized = manager._initialized
    original_engine = manager.async_engine
    connection = _RecordingConnection()
    ...
    for table in ("career_records", "visual_observations", "legal_matters", "learning_wrong_questions"):
        assert f"CREATE TABLE IF NOT EXISTS {table}" in statements
        assert f"ix_{table}_uid" in statements
