...
async def test_async_agent_run_stream_two_turns_and_persistence(
    ...
    try:
        ...
        second_event_counts = await _consume_events(e2e_client, e2e_headers, run_id)
        assert second_event_counts.get("messages", 0) > 0, second_event_counts
        assert second_event_counts.get("end", 0) == 1, second_event_counts

        second_run_payload = await _wait_for_run(e2e_client, e2e_headers, run_id)
        assert second_run_payload.get("status") == "completed", second_run_payload
        assert second_run_payload.get("request_id") == request_id
        ...
        assert first_request_id in history_text, history_text
        assert request_id in history_text, history_text
        assert history_text.count(EXPECTED_OUTPUT) >= 2, history_text

        first_run_response = await e2e_client.get(f"/api/agent/runs/{first_run_id}", headers=e2e_headers)
        assert first_run_response.status_code == 200, first_run_response.text
        assert (first_run_response.json().get("run") or {}).get("conversation_thread_id") == thread_id
        run_completed = True
    finally:
