...
async def _delete_subagent_sandbox(file_thread_id: str, skills_thread_id: str, uid: str) -> None:
    provisioner_url = os.getenv("SANDBOX_PROVISIONER_URL", "http://sandbox-provisioner:8002").rstrip("/")
    sandbox_id = sandbox_id_for_thread(file_thread_id, skills_thread_id, uid=uid)
    ...
    response = await client.post(
        ...
        json={
            ...
            "meta": {
                ...
                "source": "realtime",
                "channel": "voice",
                "realtime_session_id": f"rt-subagent-e2e-{uuid.uuid4()}",
            },
        },
        headers=headers,
    )
...
async def test_subagent_stream_records_run_and_shares_output_files(
    ...
    try:
        ...
        if thread_id and child_thread_id:
            await _delete_subagent_sandbox(thread_id, child_thread_id, uid)
