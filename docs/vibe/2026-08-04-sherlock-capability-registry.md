...
本轮验收针对“学生、法律、视觉、职场能力同时注册,按需切换且不互相泄漏”的核心目标,验证注册表、运行时门控和真实 AgentRun 链路,不把模型自动识别质量等同于代码注册成功。

| 验收项 | 验证内容 | 结果 |
| --- | --- | --- |
| 四领域 Skill 注册 | 学生、法律、视觉、职场总 Skill 及其子 Skill 均能从内置注册表读取并展开依赖 | 通过,单元测试覆盖 |
| 领域 Tool 门控 | 激活某个领域后只暴露该领域登记的业务 Tool;未激活领域不会因注册存在而自动暴露 | 通过,学生/法律/视觉/职场门控测试覆盖 |
| 统一身份 | 领域 Skill 只提供场景方法和约束,不创建或替换 Sherlock 总 Agent | 通过,注册与 Prompt 配置链路保持统一 |
| 多模态输入 | 图片入口校验模型视觉能力;实时来源可以把摄像头或屏幕帧送入标准 AgentRun | 通过,图片入口集成测试 2 项 |
...
此外,内置 Agent 幂等初始化已经兼容缺少 `config_json` 的最小对象,多模态测试显式声明视觉模型能力,职场和视觉门控测试改用显式模块引用以避免测试导入顺序污染。这些修复不改变领域架构,只提高初始化兼容性和验收稳定性。
