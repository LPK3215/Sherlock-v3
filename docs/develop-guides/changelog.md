...
- 修复实时来源的非法 AgentRun 在缺少 `realtime_session_id` 时仍先入队、再由 worker 失败的问题,现在创建接口会直接返回 422。

### 业务能力

- 将学生学习内置 Skill 调整为通用学生拍题助手:优先使用模型自身的视觉理解和推理能力,覆盖多学科、多年级及图片、摄像头、文档、语音和文字输入;知识库、联网搜索和学习记录工具按需调用,不要求导入完整教材题库。
- 新增按用户 UID 隔离的错题业务表、Repository 以及保存、查询、更新、删除 Tools;更新和删除由 Skill 规则要求用户明确确认。
- 固化夏洛克作为唯一总智能体的身份边界,明确学生、法律、编程等领域 Skill 只是可组合的能力包,并增加多 Skill 共存回归测试。
- 新增领域能力登记册,记录学生学习领域的真实依赖和法律事务领域的规划依赖,统一场景、Skill、Tool、知识源、数据库、权限和验收登记规则。
- 新增法律事务能力包:注册法律事务总 Skill、合同分析 Skill、事实整理 Skill,新增按用户 UID 隔离的法律事务记录模型、Repository 和确认式 CRUD Tools;复用现有文档、OCR、知识库和多模态入口,未导入全量法律资料库。
- 新增视觉观察与万物识别能力包:注册视觉观察、视觉识别和视觉资料核验 Skill,新增按用户 UID 隔离的视觉观察摘要表及确认式保存/查询 Tools;复用图片、实时视频帧、屏幕、知识库和搜索能力,不保存原始图片、不逐帧入库,并补充学生、法律、视觉三领域共存与依赖隔离测试。
- 新增职场与个人工作助手能力包:注册工作整理、职业文档和职业规划 Skill,新增按用户 UID 隔离的职业记录表及确认式保存/查询 Tools;复用文本、语音、图片、文档和知识库能力,不自动保存原始会话、不虚构履历或代替用户对外承诺,并补充四领域共存与工具门控测试。
- 完成职场能力第二阶段业务验收:职业记录保存接入 Yuxi 通用 interrupt/resume 确认链路,取消确认不会落库,确认后才保存;补充 CareerRepository 的 UID 隔离、筛选、排序和数量限制测试,并在运行中的 PostgreSQL 中完成创建、查询、隔离和清理验证。
- 查漏补缺并统一四领域副作用边界:学生错题保存/更新/删除、法律事务保存/更新/删除、视觉观察保存均接入 Yuxi interrupt 确认链路,取消确认不会触发 Repository 写入,并补充对应回归测试。
- 完成四领域共存运行链路验收:学生、法律、视觉和职场 Skill/Tool 同时注册并按 Skill 门控,实时来源复用标准 AgentRun,覆盖工具事件、checkpoint/history、工作区 `MEMORY.md` 和用户数据隔离;Docker 单元测试 `930 passed, 2 skipped`,图片入口集成测试 `2 passed`,实时 AgentRun 能力 E2E `1 passed`。
- 修复本轮验收稳定性问题:内置 Agent 幂等初始化兼容缺少 `config_json` 的最小对象,图片消息单测显式声明模型支持视觉输入,职场和视觉 Tool 门控测试改用显式 MCP 服务模块引用,避免测试导入顺序污染。
...
- 将实时通话后端从独立 Gateway 连接改为 Yuxi 原生二次开发：Pipecat cascade 作为 `yuxi.realtime` 依赖模块运行在现有 FastAPI 中，Yuxi 自身提供会话创建、WebRTC offer/ICE 和实时帧接口；语音或文字直接调用 `agent_run_service` 创建标准 AgentRun，并直接消费 Redis Run 事件进入 TTS，不再经过 `YuxiAgentClient`、HTTP SSE 解析或 Redis access token 中转。Yuxi 启动时按自身模型管理规范注册参考实现使用的 ModelScope `Qwen/Qwen3-VL-8B-Thinking`，并创建绑定业务提示词的 `sherlock-realtime` 内置 Agent；已有误绑定到 SiliconFlow 模型的实时 Agent 会定向迁移，管理员自行修改过的模型与 Prompt 保持不变。摄像头与共享屏幕所需的媒体路由、轨道重协商和关键帧处理同步迁入原生模块。
- 新增独立 `user-app` 用户端，与 Yuxi 管理端 `web` 共享唯一 Yuxi 后端；用户端直接调用 `/api/realtime/start` 与会话 Offer/ICE 接口，保留文字、语音、视频、屏幕共享、PiP、审批和异常重连。Compose 增加 `user-app` 服务并删除历史 `realtime-client`、`realtime-gateway` 和纯视觉参考 `user-portal`。
...
- 完成 Realtime 产品化 P0 和 P1：Gateway 凭证改为 Redis Session Registry，realtime-client `/api/start` 路由调用 Yuxi API `POST /api/realtime/session` 将 access_token 存入 Redis 并只传 session_id 给 Gateway，token 不再在 Docker 网络中明文传递（兼容回退到明文 token）；WebRTC 断连后客户端 1 秒自动重连，用户主动挂断不触发，重连状态在页面上显示"重连中…"；Gateway `run_turn` 追踪 SSE last_seq，断流后用 `after_seq` 续传最多 10 次；thread_id 从 localStorage 改为 sessionStorage 实现多标签页隔离。
